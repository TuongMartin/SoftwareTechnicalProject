		...
		if (request.getParameter("submit") != null) {
			...
			for (Account account : listAccount) {
				if (email.equals(account.getUsername()) && BCrypt.checkpw(password, account.getPassword())) {
					obj = new Account(account.getId(), email, account.getPassword(), account.getIdrole(),
							account.getrole(), account.getIdNhanVien(), account.getTenNhanVien());
					check = true;
					break;
				}
			}
			if (check) {
				...
				request.getSession().setAttribute("objUser", accountDAO.getAccountByIdSale(obj.getIdNhanVien()));
